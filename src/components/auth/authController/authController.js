...
class AuthController {
	...
	logout = async (req, res) => {
		try {
			...
			const response = {
				error: err.message || 'Error interno en el servidor durante el logout',
			};
	...
	githubCallbackRedirect = async (req, res) => {
		try {
			...
			req.logger.debug(
				`Login last_connection -> previous: ${previousLastConnection.toISOString()} -> new: ${user.last_connection.toISOString()}`
			);
			res.redirect('/products');
		} catch (error) {
			req.logger.error('Error en githubCallbackRedirect.', error);
			return res.sendServerError(
				'Ocurrió un error durante la ejecución de githubCallbackRedirect '
			);
		}
	};

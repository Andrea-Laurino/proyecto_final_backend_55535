    ...
    getProductsViews = async (limit, page, query, res) => {
        try {
            ...
            const nextLink = result.hasNextPage
                ...
                : null;
            const prevLinkAdmin = result.hasPrevPage
                ? `/admin/dashboard/products?limit=${options.limit}&page=${result.prevPage}`
                : null;
            const nextLinkAdmin = result.hasNextPage
                ? `/admin/dashboard/products?limit=${options.limit}&page=${result.nextPage}`
                : null;
            return {
        ...
        try {
            ...
            if (invalidFields.length > 0) {
                ...
                if (userData.role === 'premium' && product.owner !== userData._id) {
                    return res.sendUserError(
                        'Este producto no fue creado por ti como usuario Premium. No tienes permisos para actualizar este producto'
                    );
                }
    ...
    deleteProduct = async (pid, res, req) => {
        try {
            ...
            if (
                ...
                productOwnerIdString !== userData._id
            ) {
                return res.sendUserError(
                    'Este producto no fue creado por ti como usuario Premium. No tienes permisos para eliminar este producto'
                );
            }
        ...
        `;
                ...
                const emailPayload = {
                    ...
                    to: userEmail,
                    subject: 'THE BEAUTY - Eliminación de producto',
                    html: emailContent,
                    attachments,
                };
                try {
                    ...
                    req.logger.info(
                        `Mail enviado al user Premium ${ownerUser.email} informando la eliminación de su producto`
                    );
                } catch (error) {

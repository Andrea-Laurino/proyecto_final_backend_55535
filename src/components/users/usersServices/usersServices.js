...
class UsersServices {
        ...
        for (const userData of usersToCreate) {
            ...
            if (!existingUser) {
                ...
                try {
                    ...
                    req.logger.info(
                        `Usuario "${firstName} Role: ${role}" creado con éxito. Register for Testing last_connection -> new: ${savedUser.last_connection}`
                    );
                } catch (error) { }
            }
        }
    ...
    getPrincipalDataUser = async (req, res) => {
        try {
            ...
            const dataPromises = users.map((user) =>
                usersServices.getPrincipalDataUserDTO(user)
            );
            const data = await Promise.all(dataPromises);
            return res.sendSuccess({
                message:
                    'Todos los datos principales de los usuarios obtenidos satisfactoriamente',
                payload: data,
            });
        } catch (error) {
            return res.sendServerError(
                'Error al obtener los datos principales de los usuarios'
            );
        }
    };
    ...
    getUsers = async (limit, page, query, res) => {
        try {
            ...
            const data = {
                ...
                prevLink: users.hasPrevPage
                    ? `/users?limit=${options.limit}&page=${users.prevPage}`
                    : null,
                nextLink: users.hasNextPage
                    ? `/users?limit=${options.limit}&page=${users.nextPage}`
                    : null,
            };
            return res.sendSuccess({
    ...
    getUsersViews = async (limit, page, query, res) => {
        try {
            ...
            const prevLink = result.hasPrevPage
                ? `/admin/dashboard/users?limit=${options.limit}&page=${result.prevPage}`
                : null;
            const nextLink = result.hasNextPage
                ? `/admin/dashboard/users?limit=${options.limit}&page=${result.nextPage}`
                : null;
            return {
    ...
    registerUser = async (req, payload, res) => {
        try {
            ...
            req.logger.info(
                `Usuario"${first_name}" creado con éxito. Register last_connection -> new: ${savedUser.last_connection}`
            );
    ...
    `;
            const attachments = [
                {
                    filename: 'theBeauty.png',
                    path: path.join(__dirname, '../../../uploads/mail/theBeauty.png'),
                },
            ];
            const emailPayload = {
                from: 'andreajlaurino@gmail.com',
                to: user.email,
                subject: 'THE BEAUTY - Reestablecer de contraseña',
                html: emailContent,
                attachments,
            };
    ...
    deleteInactiveUsers = async (req, res) => {
        try {
            ...
            if (inactiveUserEmails.length > 0) {
                req.logger.info(
                    `→ Usuario/s eliminado/s de la base de datos por inactividad: "${inactiveUserEmails}"`
                );
            }
        ...
        `;
                    ...
                    const emailPayload = {
                        from: 'andreajlaurino@gmail.com',
                        to: user.email,
                        subject: 'THE BEAUTY - Cuenta eliminada por inactividad',
                        html: emailContent,
                        attachments,
                    };
                    try {
                ...
                req.logger.info(
                    '→ No se encontraron usuarios inactivos para eliminar de la base de datos'
                );
    ...
    createFakeUser = async (req, res) => {
        ...
        try {
            ...
            req.logger.info(
                `Fake user "${fakeUser.first_name}" created successfully. Register for Testing last_connection -> new: ${savedUser.last_connection}`
            );
